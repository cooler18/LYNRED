...
def map_contrast(image, method="prewitt", kernel_size=5, kernel_blur=3, low_threshold=10, ratio=3):
    ...
    # se = morphology.disk(2)  # structural element : disk of radius 2 pixels
    # m_open = morphology.opening(im, se)
...
def template_matching(image_RGB, image_IR, split=(2, 2), method=cv.TM_CCOEFF, L=50):
    # start = time.time()
    ir, rbg = size_matcher(image_IR, image_RGB)

    stepx = int(ir.shape[0] // split[0])
    stepy = int(ir.shape[1] // split[1])
    im = np.zeros_like(ir.GRAYSCALE())
    # Load the target image with the shapes we're trying to match
    for i in range(split[0]):
        for j in range(split[1]):
            template = ir[stepx * i:stepx * (i + 1), stepy * j:stepy * (j + 1)].GRAYSCALE()
            target_gray = rbg[stepx * i:stepx * (i + 1),
                          max(stepy * j - L, 0):min(stepy * (j + 1) + L, ir.shape[1])].GRAYSCALE()
            s = target_gray.shape[1] - L - stepy
            match = cv.matchTemplate(target_gray, template, method)
            delta = cv.minMaxLoc(match)[3][0] - s
            if j != split[1] - 1:
                im[stepx * i:stepx * (i + 1), stepy * j + delta:stepy * (j + 1) + delta] = template
            elif delta > 0:
                im[stepx * i:stepx * (i + 1), stepy * j + delta:-(ir.shape[1] - stepy * (j + 1))] = template[:,
                                                                                                    : -delta]
            else:
                im[stepx * i:stepx * (i + 1), stepy * j + delta: stepy * (j + 1) + delta] = template
    return ImageCustom(im, ir.GRAYSCALE())
